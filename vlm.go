...
func (instance *VLC) Vlm() (response string, err error) {
	response, _, _, err = instance.RequestMaker("/requests/vlm.xml")
	return
}
...
func (instance *VLC) VlmCmd(cmd string) (response string, err error) {
	response, _, _, err = instance.RequestMaker("/requests/vlm_cmd.xml?command=" + url.QueryEscape(cmd))
	return
}

// Get last VLM Error
func (instance *VLC) VlmCmdErr() (response string, err error) {
	response, _, _, err = instance.RequestMaker("/requests/vlm_cmd.xml")
	return
}
